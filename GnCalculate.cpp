...
int GnCalculate::configDropStone(int colour, int mov) { // 著子判斷，0不可落子，1有提子，2無提子
	...
	if ((tmpx > 0) && (ston[tmpx - 1][tmpy] == opcolour)) { // 上
		...
		if (findStoneBlock(opcolour, 4, mov - 100, tmpsiz, tmpi) == 0) { // 無氣，需要提子
			lift += tmpi;
	...
	if ((tmpy > 0) && (ston[tmpx][tmpy - 1] == opcolour) &&
		...
		if (findStoneBlock(opcolour, 4, mov - 1, tmpsiz, tmpi) == 0) { // 無氣，需要提子
			lift += tmpi;
			for (tmpi = 0; tmpi <= tmpsiz; tmpi++) {
				for (tmpj = 0; tmpj <= tmpsiz; tmpj++) {
					if (tmpbord[tmpi][tmpj] == 4) tmpbord[tmpi][tmpj] = 5;
	...
	if ((findStoneBlock(colour, 3, mov, tmpsiz, bnum) == 0) && (lift == 0)) { // 本色棋子無氣且無提子
