...
void SgfStruct::deleteroot(GnNode *tmproot) {
	if (tmproot != NULL) {
...
int SgfStruct::iswhite(char tmpc) { // 判斷空白符號
	if (tmpc == ' ' || tmpc == '\n' || tmpc == '\t' || tmpc == 13) return 1;
	return 0;
}


int SgfStruct::dealSize() {
...
int SgfStruct::dealAddStones(struct GnNode *tmpnode, int colour) {
	int tmpx, tmpy;
...
int SgfStruct::dealMove(struct GnNode *tmpnode, int colour) {
	int tmpx, tmpy;
...
int SgfStruct::dealCommentNodename(struct GnNode *tmpnode, int tmpkind) {
...
int SgfStruct::dealLabels(struct GnNode *tmpnode, int form) {
	char tmpform;
...
int SgfStruct::configNode() { // 處理一個非根節點，curNode指之
...
int SgfStruct::configManual(char *filename) {
...
int SgfStruct::reverse(int deep, struct GnNode *cur) {
